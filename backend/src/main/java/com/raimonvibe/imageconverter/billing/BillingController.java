...
public class BillingController {
    ...
    /**
     * When false, checkout returns 503 and no Stripe session is created.
     * Webhooks and UserController subscription toggles stay active for existing customers.
     * Re-enable via BILLING_PAYMENTS_ENABLED=true / app.billing.paymentsEnabled.
     */
    @Value("${app.billing.paymentsEnabled:false}")
    ...
    public ResponseEntity<Map<String, Object>> createCheckout(
        ...
        if (!paymentsEnabled) {
            logger.info("Checkout declined: app.billing.paymentsEnabled is false");
            return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
                    .body(Map.of(
                            "error", "Paid subscriptions are temporarily unavailable.",
